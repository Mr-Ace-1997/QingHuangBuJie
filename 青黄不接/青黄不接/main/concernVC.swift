...
class concernVC: UIViewController, UITableViewDataSource,
UITableViewDelegate{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "content"){
            let indexPath = sender as! IndexPath //tableCT.indexPath(for: sender as! UITableViewCell)
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        do{
            ...
            if let results = fetchedResults{
                ids.removeAll()
                ids = (results[0].value(forKey: "concerns") as? [String])!
            }
        }catch{
